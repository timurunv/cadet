...
from .preprocess import process_raw_datasets
...
class HateSpeechDataset:
    """Loader for processed hate speech datasets backed by Hugging Face Datasets.
    ...
    def __init__(
        ...
        if self.use_hf:
            ...
            # Traditional local path
            self.dataset_path = self.root / self.dataset_name
            if not self._check_exists():
                logger.info(
                    "Processed data not found. Processing raw data for %s", self.dataset_name
                )
                process_raw_datasets(root=self.root, dataset=self.dataset_name)

        self.data: Dataset | DatasetDict = self.load_data()

    def load_data(self) -> Dataset | DatasetDict:
        """Load processed hate speech datasets from local disk or HF Hub.
        ...
        Raises:
            ...
            FileNotFoundError: If processed data does not exist and raw data is missing.
        """
